...
export default function ClassList() {
        ...
        api.get("/classes")
            .then((data) => {
                ...
                const normalized = payload.map((c) => ({
                    ...
                    studentCount: Array.isArray(c.student_ids) ? c.student_ids.length : (c.students || 0),
                    description: c.description || "",
                    updatedAt: c.updated_at || c.updatedAt || c.updated_at || c.updatedAt || c.updated_at || c.created_at || "",
                    raw: c,
                }));
    ...
    const toggleLectures = (classId) => {
        setLecturesMap((m) => ({ ...m, [classId]: { ...(m[classId] || {}), open: !m[classId]?.open } }));
        ...
        setLecturesMap((m) => ({ ...m, [classId]: { ...(m[classId] || {}), loading: true, error: null } }));

        api.get(`/lectures/class/${classId}`)
            .then((res) => {
                const payload = Array.isArray(res) ? res : Array.isArray(res?.data) ? res.data : [];
                setLecturesMap((m) => ({ ...m, [classId]: { ...(m[classId] || {}), loading: false, data: payload } }));
            })
            .catch((err) => {
                console.error("Failed to fetch lectures for class", classId, err);
                setLecturesMap((m) => ({ ...m, [classId]: { ...(m[classId] || {}), loading: false, error: err?.message || 'Lỗi khi tải bài giảng' } }));
            });
    };
        ...
        <div className="p-6">
            <div className="max-w-7xl mx-auto">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 text-gray-500">Không tìm thấy lớp nào.</div>
                ) : (
                        ...
                        {filtered.map((c) => (
                            ...
                            >
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="text-sm text-gray-400">{c.studentCount} sv</div>
                                </div>

                                <p className="mt-3 text-sm text-gray-600 min-h-[48px]">{c.description || '-'}</p>

                                <div className="mt-4 flex items-center justify-between">
                                    <div className="text-xs text-gray-400">Cập nhật: {c.updatedAt ? new Date(c.updatedAt).toLocaleString() : '-'}</div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            onClick={() => alert('Chia sẻ/Quản lý lớp - chưa triển khai')}
                                            className="px-3 py-1 text-sm bg-gray-100 rounded hover:bg-gray-200"
                                        >
                                            Quản lý
                                        </button>
                                ...
                                {lecturesMap[c.id]?.open && (
                                    <div className="mt-3 border-t pt-3">
                                        {lecturesMap[c.id]?.loading ? (
                                            ...
                                            <div className="text-sm text-red-500">Lỗi: {lecturesMap[c.id].error}</div>
                                        ) : Array.isArray(lecturesMap[c.id]?.data) && lecturesMap[c.id].data.length === 0 ? (
                                            <div className="text-sm text-gray-500">Chưa có bài giảng.</div>
                                        ) : (
                                            <ul className="space-y-2">
                                                {lecturesMap[c.id]?.data?.map((lec) => (
                                                    <li key={lec._id || lec.id} className="text-sm flex items-center justify-between">
                                                        <div>
                                                            <div className="font-medium">{lec.title || lec.name || 'Bài giảng'}</div>
                                                            <div className="text-xs text-gray-400">{lec.updated_at ? new Date(lec.updated_at).toLocaleString() : (lec.created_at ? new Date(lec.created_at).toLocaleString() : '')}</div>
                                                        </div>
                                                        <div>
                                                            {(lec.pdf_url || lec.pdfUrl) ? (
                                                                <button
                                                                    onClick={() => {
                                                                        try {
                                                                            ...
                                                                            console.error('Failed to set pdfUrl in localStorage', err);
                                                                        }
                                                                        navigate('/lecture');
                                                                    }}
                                                                ...
                                                                <button disabled className="ml-3 px-2 py-1 text-xs bg-gray-200 text-gray-500 rounded">Không có PDF</button>
                                                            )}
                                                        </div>
                                                    </li>
                                                ))}
